...
def task_checker(task_list: list): #Список ошибок
...
def duration(task_list: list, executor_count: int) -> int: #Вычисление продолжительности
    all_dur = list(map(lambda t: t[1], task_list))
    t_max = max(all_dur)
    t_avg = sum(all_dur)//executor_count
    t_opt = max(t_max, t_avg) #Вычисляем максимальное среди среднего времени по всем работам и максимального
    return t_opt

def downtime(executor_tasks: list, duration: int) -> int: #Высчитываем время простоя
    downtime_sum = 0
    for executor in executor_tasks:
        downtime_sum += duration - sum(map(lambda d: d[PART], executor))
    return downtime_sum

def downtime_for_executor(executor_idx: int, executor_tasks: list, duration: int) -> int: #Высчитываем время простоя для конкретного исполнителя
    return duration - sum(map(lambda d: d[PART], executor_tasks[executor_idx]))


def distribute_tasks(task_list: list, executor_count: int, duration: int): #Раздаем задачи исполнителям
...
def schedule_for_executor(executor_idx: int, executor_tasks: list, duration: int) -> str: #Расписание для кокретного исполнтеля
    answer = ""
    hours = 0
...
def create_schedule(task_list: list, executor_count: int): #Составляем итоговое расписание со всеми параметрами
    task_checker(task_list) #Проверка на ошибки
    ...
    dur = duration(task_list, executor_count) #Продолжительность
    executor_tasks = distribute_tasks(task_list, executor_count, dur) #Задачи исполнителей
    dtime = downtime(executor_tasks, dur) #Время простоя
    schedule = {
